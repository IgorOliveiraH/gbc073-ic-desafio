...
Original file is located at
    https://colab.research.google.com/drive/11XX26IYtj1VbUFmqKC2nBo8xW77IQ4vf
"""
...
class Submissao:
    ...
    def fit(self, X: torch.Tensor) -> None:
        ...
        amostra = Xs[torch.randperm(len(Xs), generator=g)[:min(300, len(Xs))]]
        d2 = torch.cdist(amostra, amostra) ** 2
        mediana = d2[d2 > 0].median().item() if (d2 > 0).any() else 1.0
        sigma_base = math.sqrt(mediana / 2.0) if mediana > 0 else 1.0

        budget = self.DIM_MAX - 2 * d
        n_pares_rff = max(budget // 2, 1)

        escalas = torch.tensor([0.2, 0.5, 1.0, 2.5])
        bloco = n_pares_rff // len(escalas)
        sigmas = []
        for i, esc in enumerate(escalas):
            qtd = bloco if i < len(escalas) - 1 else (n_pares_rff - bloco * (len(escalas) - 1))
            if qtd > 0:
                sigmas.extend([sigma_base * esc.item()] * qtd)
        sigmas = torch.tensor(sigmas)
        n_pares_rff = len(sigmas)

        W_list = []
        restante = n_pares_rff
        while restante > 0:
            k = min(restante, d)
            q, _ = torch.linalg.qr(torch.randn(d, d, generator=g))
            raios = torch.sqrt((torch.randn(d, k, generator=g) ** 2).sum(0))
            W_list.append(q[:, :k] * raios)
            restante -= k
        W_raw = torch.cat(W_list, dim=1)
    ...
    def phi(self, X: torch.Tensor) -> torch.Tensor:
        ...
        fourier = (torch.cat([torch.cos(proj), torch.sin(proj)], dim=1)
                   * math.sqrt(1.0 / self.W.shape[1])).float()

        return torch.cat([Xs, Xs ** 2, fourier], dim=1)
# =============================================================================
...
def perceptron_pocket(Z, y, epocas=50, eta=1.0, semente=0):
    ...
    yb = 2 * y - 1
    w = torch.zeros(Zb.shape[1]);
    melhor_w, melhor_acc = w.clone(), -1.0
...
def avaliar():
    ...
    for nome, gen in TAREFAS.items():
        ...
        try:
            m = _mediana(Submissao, gen);
            erro = ""
        except AssertionError as e:
            m, erro = b, f"   <- {e}"
        st = min(max((m - b) / (r - b), 0.0), 1.25);
        s.append(st)
